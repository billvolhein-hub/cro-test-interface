    ...
    <div style={{ background: CARD, border: `1.5px solid ${BORDER}`, borderRadius: 10, marginBottom: 24, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,.06)" }}>
      ...
      >
        ...
        <div style={{ flex: 1, fontSize: 12, fontWeight: 700, color: NOTE_COLOR, letterSpacing: 0.5 }}>
          Client Notes
          <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 600, color: "#fff", background: NOTE_COLOR, borderRadius: 10, padding: "1px 8px" }}>{notes.length}</span>
        </div>
        {onToggle && (
        ...
        <div style={{ maxHeight: 340, overflowY: "auto" }}>
          {notes.map(({ test, overlay }) => {
            ...
            return (
              <div
                key={overlay.id}
                style={{ display: "flex", gap: 12, padding: "12px 18px", borderBottom: `1px solid ${BORDER}`, alignItems: "flex-start" }}
              >
